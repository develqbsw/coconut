...
import sk.qbsw.security.ocsp.service.sha256.SHA256CertificateID;
...
@Service("ocspbcimpl")
public class OCSPCertValidatorBCImpl implements OCSPCertValidator {
	...
	public byte[] generateOCSPRequest(X509Certificate issuerCert, BigInteger serialNumber) {
		...
		try {
			...
			CertificateID id = new SHA256CertificateID(issuerCert, serialNumber);
	 ...
	 */
	private OCSPResponseStatus getOCSPStatus(OCSPResp ocspResponse) {
		return OCSPResponseStatus.valueOf(ocspResponse.getStatus());
	}
	...
	public OCSPValidationResult processResponse(InputStream ocspResponseStream) {
		OCSPValidationResult retVal = new OCSPValidationResult();

		try {
			OCSPResp ocspResponse = new OCSPResp(ocspResponseStream);
			OCSPResponseStatus ocspResponseStatus = getOCSPStatus(ocspResponse);
			retVal.setOcspStatus(ocspResponseStatus);

			if (OCSPResponseStatus.SUCCESSFUL.equals(ocspResponseStatus)) {
